...
abstract class Defender_Core {
	 ...
	 * @param string|Config_Group $config Конфигурационные данные модуля или имя конфигурации.
	...
	public static function login(string $userName, string $password, $config = 'defender') : Defender {
		try {
			...
			if(self::check($password, $_user->{self::$config['uattr']['password']})) { // Если пароль успешно проверен, то завершаем аутентификацию
				...
				self::$sess
						->set(self::$config['session']['key'], $userName)
						->set(self::$config['session']['key'].'_TIME', time() + self::$config['session']['expiration']); // Запоминаем в сессии имя пользователя и время завершения сеанса пользователя по бездействию
				return $_defender;
	...
	public static function getSession() : Session {
		return self::$sess; // Возвращаем текущую сессию
	}
	/**
	...
	protected static function getUserModel($user) {
		self::initConfig(); // Инициализируем конфигурацию
		$_model = null;
	...
	/**
	 * Восстанавливает состояние объекта после сериализации.
	 */
	public function __wakeup() {
		if(!is_object($_user) || !self::$config['session']['store_user']) {
			$this->logout();
			$this->roles = $this->rules = array();
		}
	}
	/**
	 * Возвращает объект, соответствующий пользователю (если таковой имеется), в противном случае вернет false.
	 * @return object
	 */
	...
	public function isAllowed(string $control = '', string $action = '') : bool {
		...
		if(array_key_exists('*', $this->rules)) { // Если указан подстановочный символ, значит у пользователя неограниченный доступ ко всем контролам и действиям
			return true;
		} else if(array_key_exists($control, $this->rules)) {
			if(in_array('*', $this->rules[$control], true)) // Если указан полный доступам ко всем действияем внутри контрола
				return true;
			else if(in_array($action, $this->rules[$control], true)) // Если разрешен доступ к контролу и действию данного контрола
				return true;
		}
